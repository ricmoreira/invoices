...
func NewDBCollections(config *config.Config) *DBCollections {
	...
	// set invoices index
	keys, err := bson.ParseExtJSONObject(`{ "InvoiceNo": 1}`)
	options, err := bson.ParseExtJSONObject(`{ "unique": true }`)
	if err != nil {
		log.Fatal(err)
	}
 
	invoicesIndex := mongo.IndexModel{
		Keys:    keys,
		Options: options,
	}

	invoiceCollection := db.Collection("invoices")
	_, err = invoiceCollection.Indexes().CreateOne(context.Background(), invoicesIndex)
	if err != nil {
		log.Fatal(err)
	}
